...
export default async function MovementHistoryReportPage({
  ...
  const orgHasMovements =
    hasAnyMovements ||
    (await db.stockMovement.count({ where: { organizationId: membership.organizationId } })) > 0;
          ...
          <div className="flex items-center gap-2">
            {canExport && visibleMovements.length > 0 ? (
              <Button asChild variant="outline">
                <Link href={buildExportHref(filter.rawFrom, filter.rawTo, filter.rawType)}>
      ...
      {visibleMovements.length === 0 ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {[
              { label: t.reports.movementHistory.kpiTotalMovements, value: totalMovements },
              { label: t.reports.movementHistory.kpiReceipts, value: receipts.length },
              { label: t.reports.movementHistory.kpiIssues, value: issues.length },
              { label: t.reports.movementHistory.kpiTransfers, value: transfers.length },
              { label: t.reports.movementHistory.kpiAdjustments, value: adjustments.length },
              {
              ...
              <div className="flex items-center justify-between">
                ...
                {truncated ? (
                  <CardDescription className="text-xs">
                    {format(t.movements.filter.truncatedNotice, { limit: String(PAGE_SIZE) })}
                  </CardDescription>
                ) : null}
              </div>
            </CardHeader>
