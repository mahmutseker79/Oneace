...
export default async function StockValueReportPage() {
	...
	const itemsMissingCost = new Set(
		rows.filter((r) => r.costPrice == null).map((r) => r.id),
	).size;
			...
			<div className="space-y-1">
				...
				<div className="flex items-start justify-between gap-4">
					<div className="flex items-start gap-3">
						...
						<div>
							<h1 className="text-2xl font-semibold">
								{t.reports.stockValue.heading}
							</h1>
							<p className="text-muted-foreground">
								{t.reports.stockValue.subtitle}
							</p>
						</div>
					</div>
					<div className="flex items-center gap-2">
						...
						<ExportButton href="/reports/stock-value/export">
							{t.common.exportCsv}
						</ExportButton>
					</div>
				</div>
				...
				<>
					<div className="grid gap-4 md:grid-cols-3">
						<Card>
							<CardHeader>
								<CardDescription>
									{t.reports.stockValue.totalValueLabel}
								</CardDescription>
						...
						<Card>
							<CardHeader>
								<CardDescription>
									{t.reports.stockValue.totalUnitsLabel}
								</CardDescription>
								<CardTitle className="text-3xl font-mono">
						...
						<Card>
							<CardHeader>
								<CardDescription>
									{t.reports.stockValue.distinctItemsLabel}
								</CardDescription>
								<CardTitle className="text-3xl font-mono">
								...
								<Table>
									...
									<TableBody>
										{group.items
											.slice()
											.sort(
												(a, b) => (b.valueAtCost ?? 0) - (a.valueAtCost ?? 0),
											)
											.map((item) => (
												<TableRow key={`${item.id}-${item.warehouseId}`}>
													...
													<TableCell className="text-right font-mono">
														{formatNumber(item.onHand, region.numberLocale)}{" "}
														{item.unit}
													</TableCell>
													<TableCell className="text-right font-mono">
														{item.costPrice == null
