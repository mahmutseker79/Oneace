...
export default async function ItemDetailPage({
	...
	const movements = hasMoreMovements
		? movementsRaw.slice(0, MOVE_PAGE_SIZE)
		: movementsRaw;
	const nextMoveCursor = hasMoreMovements
		...
		<div className="space-y-6">
					...
					<CardContent className="space-y-3 text-sm">
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.itemDetail.skuLabel}
							</span>
							<span className="font-mono">{item.sku}</span>
						</div>
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.itemDetail.barcodeLabel}
							</span>
						...
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.itemDetail.categoryLabel}
							</span>
							<span>{item.category?.name ?? t.itemDetail.noCategory}</span>
						</div>
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.itemDetail.unitLabel}
							</span>
							<span>{item.unit}</span>
						</div>
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.itemDetail.reorderPointLabel}
							</span>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						{totalOnHand} {item.unit} · {totalAvailable}{" "}
						{t.itemDetail.stockColumnAvailable}
					</CardDescription>
				</CardHeader>
				<CardContent className="p-0">
					{item.stockLevels.length === 0 ? (
						<p className="px-6 pb-6 text-sm text-muted-foreground">
							{t.itemDetail.stockEmpty}
						</p>
					) : (
						<Table>
							<TableHeader>
								<TableRow>
									...
									<TableHead className="text-right">
										{t.itemDetail.stockColumnQuantity}
									</TableHead>
							...
							<TableBody>
								{item.stockLevels.map((lvl) => (
									<TableRow key={lvl.id}>
										<TableCell>
											<Link
												href={`/warehouses/${lvl.warehouse.id}`}
