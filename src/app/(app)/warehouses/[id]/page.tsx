    ...
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        badge={
          <>
            {warehouse.isDefault ? (
              <Badge variant="outline">{t.warehouses.detail.defaultBadge}</Badge>
            ) : null}
            {warehouse.isArchived ? (
              <Badge variant="secondary">{t.warehouses.detail.archivedBadge}</Badge>
            ) : null}
          </>
        }
        breadcrumb={[
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t.warehouses.detail.metaHeading}</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 text-sm md:grid-cols-2">
          <div className="flex justify-between">
            <span className="text-muted-foreground">{t.warehouses.detail.metaCode}</span>
            <span className="font-mono">{warehouse.code}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">{t.warehouses.detail.metaAddress}</span>
            <span className="max-w-[70%] text-right">{warehouse.address ?? "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">{t.warehouses.detail.metaCity}</span>
            <span>{warehouse.city ?? "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">{t.warehouses.detail.metaRegion}</span>
            <span>{warehouse.region ?? "—"}</span>
          </div>
          <div className="flex justify-between md:col-span-2">
            <span className="text-muted-foreground">{t.warehouses.detail.metaCountry}</span>
            <span>{warehouse.country ?? "—"}</span>
          </div>
          <div className="flex justify-between md:col-span-2">
            <span className="text-muted-foreground">{"Code"}</span>
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs">{warehouse.barcodeValue ?? "—"}</span>
              {canEditWarehouse ? (
                <Button variant="ghost" size="sm" title="Assign code" disabled>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t.warehouses.detail.stockHeading}</CardTitle>
        </CardHeader>
                ...
                {stockLevels.map((lvl) => (
                  <TableRow key={lvl.id}>
                    <TableCell className="font-mono text-xs">
                      <Link href={`/items/${lvl.item.id}`} className="hover:underline">
                        {lvl.item.sku}
                      </Link>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right tabular-nums">{lvl.reservedQty}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t.warehouses.detail.movementsHeading}</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {movements.length === 0 ? (
            <p className="px-6 pb-6 text-sm text-muted-foreground">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t.warehouses.detail.movementsColumnDate}</TableHead>
                  <TableHead>{t.warehouses.detail.movementsColumnType}</TableHead>
                  <TableHead>{t.warehouses.detail.movementsColumnItem}</TableHead>
                  <TableHead className="text-right">
              ...
              <TableBody>
                {movements.map((m) => {
                  ...
                  const isIncomingTransfer = m.type === "TRANSFER" && m.toWarehouseId === id;
                  ...
                  return (
                    <TableRow key={m.id}>
                      ...
                      <TableCell>
                        <Link href={`/items/${m.item.id}`} className="hover:underline">
                          {m.item.name}
                        </Link>
