...
export default async function SupplierDetailPage({ params }: DetailPageProps) {
	...
	const onTimeRate =
		onTimeEligible === 0 ? null : (onTimeCount / onTimeEligible) * 100;
	const avgLead = avg(leadTimeSampleDays);

	const showCurrencyCaveat =
		currencyMix.size > 1 ||
		Array.from(currencyMix).some((c) => c !== region.currency);
		...
		<div className="space-y-6">
			...
			<div className="grid gap-4 md:grid-cols-3">
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="text-base">
							{t.suppliers.detail.contactHeading}
						</CardTitle>
					</CardHeader>
				...
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="text-base">
							{t.suppliers.detail.addressHeading}
						</CardTitle>
					</CardHeader>
					<CardContent className="text-sm">
				...
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="text-base">
							{t.suppliers.detail.notesHeading}
						</CardTitle>
					</CardHeader>
					<CardContent className="text-sm">
			...
			<div>
				<h2 className="text-lg font-semibold">
					{t.suppliers.detail.activityHeading}
				</h2>
				<p className="text-muted-foreground text-sm">
					{t.suppliers.detail.activitySubtitle}
				</p>
			</div>
			...
			{!hasActivity ? (
				<Card>
					<CardHeader className="items-center text-center">
						...
						<CardDescription>
							{t.suppliers.detail.emptyActivityBody}
						</CardDescription>
					</CardHeader>
				...
				<>
					<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
						<Card>
							<CardHeader className="pb-2">
								<CardDescription>
									{t.suppliers.detail.kpiReceivedValueLabel}
								</CardDescription>
						...
						<Card>
							<CardHeader className="pb-2">
								<CardDescription>
									{t.suppliers.detail.kpiTotalPosLabel}
								</CardDescription>
								<CardTitle className="text-2xl">
						...
						<Card>
							<CardHeader className="pb-2">
								<CardDescription>
									{t.suppliers.detail.kpiOnTimeRateLabel}
								</CardDescription>
								<CardTitle className="text-2xl">
									{onTimeRate == null
						...
						<Card>
							<CardHeader className="pb-2">
								<CardDescription>
									{t.suppliers.detail.kpiAvgLeadTimeLabel}
								</CardDescription>
								<CardTitle className="text-2xl">
									{avgLead == null
										? t.suppliers.detail.kpiNotAvailable
					...
					<Card>
						<CardHeader>
							<div className="flex flex-wrap items-center justify-between gap-2">
								<div>
									<CardTitle className="text-base">
										{t.suppliers.detail.recentHeading}
									</CardTitle>
									<CardDescription>
										{t.suppliers.detail.recentSubtitle}
									</CardDescription>
								</div>
								...
								<TableHeader>
									<TableRow>
										...
										<TableHead className="text-right">
											{t.suppliers.detail.colLines}
										</TableHead>
										<TableHead className="text-right">
											{t.suppliers.detail.colValue}
										</TableHead>
									</TableRow>
								</TableHeader>
								<TableBody>
									{recentPos.map((po) => {
										...
										const poValue = po.lines.reduce(
											(sum, line) =>
												sum +
												line.receivedQty * Number(line.unitCost.toString()),
											0,
										);
										...
										if (
											...
											const diff =
												po.receivedAt.getTime() - po.expectedAt.getTime();
											if (diff > 0) {
												timelinessBadge = (
													<Badge variant="destructive">
														{t.suppliers.detail.lateBadge}
													</Badge>
												);
											} else if (diff < 0) {
												timelinessBadge = (
													<Badge variant="secondary">
														{t.suppliers.detail.earlyBadge}
													</Badge>
												);
											} else {
												timelinessBadge = (
													<Badge>{t.suppliers.detail.onTimeBadge}</Badge>
												);
											}
										} else if (
											status === "SENT" ||
											...
											timelinessBadge = (
												<Badge variant="outline">
													{t.suppliers.detail.outstandingBadge}
												</Badge>
											);
										}
					...
					<Card>
						<CardHeader>
							<CardTitle className="text-base">
								{t.suppliers.detail.topItemsHeading}
							</CardTitle>
							<CardDescription>
								{t.suppliers.detail.topItemsSubtitle}
							</CardDescription>
						</CardHeader>
						<CardContent>
							{topItems.length === 0 ? (
