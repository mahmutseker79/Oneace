...
export default async function StockCountDetailPage({
	...
	const itemIds = Array.from(
		new Set([
			...count.snapshots.map((s) => s.itemId),
			...count.entries.map((e) => e.itemId),
		]),
	);
	...
	for (const variance of varianceRows) {
		countedByScope.set(
			`${variance.itemId}:${variance.warehouseId}`,
	...
	const visibleEntries = hasMoreEntries
		? count.entries.slice(0, ENTRY_PAGE_SIZE)
		: count.entries;
	const nextEntryCursor = hasMoreEntries
		? visibleEntries[visibleEntries.length - 1]?.id
		: null;

	const offlineRows: StockCountSnapshotRowInput[] = count.snapshots.map(
		(snapshot) => {
			const item = itemById.get(snapshot.itemId);
			const warehouse = warehouseById.get(snapshot.warehouseId);
			return {
				...
				countedQuantity:
					countedByScope.get(`${snapshot.itemId}:${snapshot.warehouseId}`) ?? 0,
			};
		},
	);
	...
	function varianceBadge(status: VarianceStatus) {
		if (status === "match")
			return <Badge variant="outline">{t.stockCounts.variance.match}</Badge>;
		if (status === "within_tolerance")
			return (
				<Badge variant="secondary">
					{t.stockCounts.variance.withinTolerance}
				</Badge>
			);
		if (status === "over")
			return (
				<Badge className="bg-emerald-600">{t.stockCounts.variance.over}</Badge>
			);
			...
			{state === "IN_PROGRESS" && count.snapshots.length > 0
				? (() => {
						...
						const countedPairs = new Set(
							fullEntries.map((e) => `${e.itemId}::${e.warehouseId}`),
						);
						const snapshotPairs = count.snapshots.map(
							(s) => `${s.itemId}::${s.warehouseId}`,
						);
						const countedCount = snapshotPairs.filter((p) =>
							countedPairs.has(p),
						).length;
				...
				<CardContent className="grid gap-4 pt-6 text-sm md:grid-cols-2 lg:grid-cols-3">
					<div className="flex justify-between">
						<span className="text-muted-foreground">
							{t.stockCounts.detail.metaState}
						</span>
						<span>{t.stockCounts.statusBadge[state]}</span>
					</div>
					<div className="flex justify-between">
						<span className="text-muted-foreground">
							{t.stockCounts.detail.metaMethodology}
						</span>
						<span>{t.stockCounts.methodology[methodology]}</span>
					</div>
					<div className="flex justify-between">
						<span className="text-muted-foreground">
							{t.stockCounts.detail.metaCreated}
						</span>
					...
					{count.startedAt ? (
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.stockCounts.detail.metaStarted}
							</span>
							<span className="whitespace-nowrap">
								{dateFormatter.format(count.startedAt)}
							</span>
						</div>
					) : null}
					{count.completedAt ? (
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.stockCounts.detail.metaCompleted}
							</span>
							<span className="whitespace-nowrap">
								{dateFormatter.format(count.completedAt)}
							</span>
						</div>
					) : null}
					{count.cancelledAt ? (
						<div className="flex justify-between">
							<span className="text-muted-foreground">
								{t.stockCounts.detail.metaCancelled}
							</span>
							<span className="whitespace-nowrap">
								{dateFormatter.format(count.cancelledAt)}
							</span>
						</div>
					) : null}
					{count.cancelReason ? (
						<div className="flex justify-between md:col-span-2 lg:col-span-3">
							<span className="text-muted-foreground">
								{t.stockCounts.detail.metaReason}
							</span>
							...
							<Table className="min-w-[640px]">
								<TableHeader>
									<TableRow>
										...
										<TableHead>
											{t.stockCounts.detail.columnWarehouse}
										</TableHead>
										{isBlind ? null : (
										...
										{isBlind ? null : (
											<>
												...
												<TableHead>
													{t.stockCounts.detail.columnStatus}
												</TableHead>
											</>
										)}
									</TableRow>
					...
					{visibleEntries.length === 0 ? (
						...
						<Table>
							<TableHeader>
								<TableRow>
									<TableHead>
										{t.stockCounts.detail.entriesColumnWhen}
									</TableHead>
									<TableHead>
										{t.stockCounts.detail.entriesColumnItem}
									</TableHead>
									<TableHead>
										{t.stockCounts.detail.entriesColumnWarehouse}
									</TableHead>
									...
									<TableHead>
										{t.stockCounts.detail.entriesColumnNote}
									</TableHead>
							...
							<TableBody>
								{visibleEntries.map((entry) => {
									...
									const user = entry.countedByUserId
										? userById.get(entry.countedByUserId)
										: null;
									return (
										<TableRow key={entry.id}>
											...
											<TableCell className="text-xs text-muted-foreground">
												{entry.binId
													? (binById.get(entry.binId)?.code ?? entry.binId)
													: "—"}
											</TableCell>
											<TableCell className="text-right tabular-nums">
