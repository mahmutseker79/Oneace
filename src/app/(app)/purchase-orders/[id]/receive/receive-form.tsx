 ...
 */
export function matchLineByCode(
	lines: ReceiveLine[],
	code: string,
): ReceiveLine | null {
	...
	const byBarcode = lines.find(
		(l) => l.itemBarcode !== null && l.itemBarcode === trimmed,
	);
...
export function ReceiveForm({
	...
	const [scanStatus, setScanStatus] = useState<
		"idle" | "matched" | "not-found" | "already-full"
	>("idle");
	...
	const pctReceived =
		totalOrdered > 0
			? Math.min(
					100,
					Math.round(
						((totalPreviouslyReceived + totalInCurrentForm) / totalOrdered) *
							100,
					),
				)
			: 0;
						...
						<TableHeader>
							<TableRow>
								...
								<TableHead className="text-right">
									{labels.columnOrdered}
								</TableHead>
								<TableHead className="text-right">
									{labels.columnReceived}
								</TableHead>
								<TableHead className="text-right">
									{labels.columnOpen}
								</TableHead>
								<TableHead className="w-[140px] text-right">
									{labels.columnReceiveNow}
								</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
									...
									>
										...
										<TableCell className="text-right">
											<Input
												...
												onChange={(e) =>
													updateQuantity(line.id, e.target.value)
												}
