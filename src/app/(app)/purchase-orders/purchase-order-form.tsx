...
export function PurchaseOrderForm({
	...
	const initialSupplierId =
		initial?.supplierId ?? prefill?.supplierId ?? suppliers[0]?.id ?? "";
	...
	const [currency, setCurrency] = useState(
		initial?.currency ??
			suppliers.find((s) => s.id === initialSupplierId)?.currency ??
			"USD",
	);
		...
		<form className="space-y-6" onSubmit={handleSubmit}>
			...
			<div className="grid gap-6 md:grid-cols-3">
				...
				<div className="space-y-2">
					...
					<Select
						value={status}
						onValueChange={(value) => setStatus(value as InitialPo["status"])}
					>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
								...
								>
									<div className="space-y-1">
										...
										>
											<SelectTrigger>
												<SelectValue
													placeholder={labels.fields.lineItemPlaceholder}
												/>
											</SelectTrigger>
											<SelectContent>
												{items.map((item) => (
