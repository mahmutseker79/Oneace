...
import { serializeCsv, csvResponse } from "@/lib/csv";
 ...
 */
async function handleGetCounts(req: Request) {
	try {
	...
	try {
		...
		const [count1, count2] = await Promise.all([
			db.stockCount.findUnique({
				where: { id: count1Id },
				include: { entries: { select: { itemId: true, countedQuantity: true, item: { select: { sku: true, name: true } } } } },
			}),
			db.stockCount.findUnique({
				where: { id: count2Id },
				include: { entries: { select: { itemId: true, countedQuantity: true, item: { select: { sku: true, name: true } } } } },
		...
		// Verify ownership
		if (count1.organizationId !== membership.organizationId || count2.organizationId !== membership.organizationId) {
	...
	try {
		...
		if (format === "csv") {
			const csv = serializeCsv(comparison, [
				...
				{ header: count1Name || "Count 1", value: (r: Record<string, unknown>) => (r.count1Qty ?? "") as string | number | null },
				{ header: count2Name || "Count 2", value: (r: Record<string, unknown>) => (r.count2Qty ?? "") as string | number | null },
				{ header: "Variance", value: (r: Record<string, unknown>) => r.variance as number },
				{ header: "Variance %", value: (r: Record<string, unknown>) => ((r.variancePercent as number).toFixed(1)) },
			]);
		...
		if (format === "xlsx") {
			const workbook = await buildExcelWorkbook(
				...
				comparison,
				[
					{ header: "SKU", key: "sku", value: (r: Record<string, unknown>) => r.sku as string, width: 12 },
					{ header: "Name", key: "name", value: (r: Record<string, unknown>) => r.name as string, width: 25 },
					{ header: count1Name || "Count 1", key: "c1", value: (r: Record<string, unknown>) => (r.count1Qty ?? "") as string | number | null, width: 10 },
					{ header: count2Name || "Count 2", key: "c2", value: (r: Record<string, unknown>) => (r.count2Qty ?? "") as string | number | null, width: 10 },
					{ header: "Variance", key: "var", value: (r: Record<string, unknown>) => r.variance as number, width: 10 },
					{ header: "Variance %", key: "varpct", value: (r: Record<string, unknown>) => r.variancePercent as number, width: 10, numFmt: "0.0%" },
				],
			);
