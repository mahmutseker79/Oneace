...
async function handleGetVariance(req: Request) {
	try {
		...
		const varianceData = departments.map((dept) => {
			...
			for (const item of deptItems) {
				...
				const counted = counts.find((c: typeof counts[number]) => c.itemId === item.id)?.countedQuantity || 0;
				countedQty += counted;
			}
	...
	try {
		...
		if (format === "csv") {
			const csv = serializeCsv(data, [
				...
				{ header: "Variance %", value: (r: Record<string, unknown>) => ((r.variancePercent as number).toFixed(2)) },
		...
		if (format === "xlsx") {
			const workbook = await buildExcelWorkbook(
				...
				data,
				[
					{ header: "Department", key: "dept", value: (r: Record<string, unknown>) => r.departmentName as string, width: 20 },
					{ header: "Items", key: "items", value: (r: Record<string, unknown>) => r.itemCount as number, width: 10 },
					{ header: "Variance", key: "var", value: (r: Record<string, unknown>) => r.variance as number, width: 12 },
					{ header: "Variance %", key: "varpct", value: (r: Record<string, unknown>) => r.variancePercent as number, width: 12, numFmt: "0.00%" },
					{ header: "Status", key: "status", value: (r: Record<string, unknown>) => r.status as string, width: 10 },
				],
			);
