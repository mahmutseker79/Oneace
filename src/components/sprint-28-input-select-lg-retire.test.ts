...
//
// Pinned guard:
//   1) Input cva'da `lg:` size value yok
//   2) SelectTrigger cva'da `lg:` size value yok
//   3) `--control-h-lg` CSS token globals.css'te HÂLÂ var (Button.lg yaşar)
//   4) Input + SelectTrigger source'da `--control-h-lg` reference yok
//   5) `<Input size="lg">` ve `<SelectTrigger size="lg">` repo'da 0 kullanım
//      (regresyona karşı hard guard).
...
describe("Sprint 28 — Input + SelectTrigger lg size retire", () => {
  it("Input cva size union: lg variant kaldırıldı", () => {
    ...
    // primitive source'da `var(--control-h-lg)` actual CSS usage yok
    // (comment metninde token adı geçebilir — sadece gerçek CSS expression yasak)
  ...
  it("Button.lg hâlâ aktif (10+ instance) — primitive variant kaldırılmadı", () => {
