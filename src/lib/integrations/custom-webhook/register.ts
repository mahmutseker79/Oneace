...
export const CUSTOM_WEBHOOK_TASK_KINDS = [
  "sync_items",
  "sync_orders",
  "sync_stock",
] as const;
...
async function runCustomWebhookSync(
  task: ClaimedTask,
  kind: CustomWebhookTaskKind,
): Promise<void> {
  ...
  if (!integration) {
    ...
    (err as { code?: string }).code =
      "SCHEMA_CUSTOM_WEBHOOK_INTEGRATION_NOT_FOUND";
    throw err;
  }
  ...
  (err as { code?: string }).code =
    "TRANSPORT_CUSTOM_WEBHOOK_EXECUTION_PENDING";
  throw err;
}

for (const kind of CUSTOM_WEBHOOK_TASK_KINDS) {
  registerHandler("custom-webhook", kind, (task) =>
    runCustomWebhookSync(task, kind),
  );
}
