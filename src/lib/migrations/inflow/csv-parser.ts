...
export function parseInflowCSV(
  ...
  for (const row of productsRows) {
    ...
    const reorderPoint = parseFloat(row["ReorderPoint"] || "0");
  ...
  for (const lineRow of poLineRows) {
    ...
    const qty = parseFloat(lineRow["Quantity"] || "0");
    const cost = parseFloat(lineRow["UnitCost"] || "0");
