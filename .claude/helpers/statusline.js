...
function getSwarmStatus() {
  ...
  try {
    const ps = execSync('ps aux 2>/dev/null | grep -c agentic-flow || echo "0"', {
      encoding: "utf-8",
    });
    activeAgents = Math.max(0, Number.parseInt(ps.trim()) - 1);
...
function getSystemMetrics() {
  ...
  try {
    ...
    memoryMB = Number.parseInt(mem.trim()) || 0;
  } catch (e) {
    // Fallback
  ...
  try {
    ...
    subAgents = Math.max(0, Number.parseInt(agents.trim()) - 1);
  } catch (e) {
    // Ignore
  }
