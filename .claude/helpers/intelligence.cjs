  ...
  try {
    ...
    if (stat.size > MAX_DATA_FILE_SIZE) {
      process.stderr.write(
        `[INTELLIGENCE] WARN: Skipping ${path.basename(filePath)} (${Math.round(stat.size / 1048576)}MB exceeds 10MB limit)\n`,
      );
      return null;
    }
  } catch {
...
function buildEdges(entries) {
  ...
  const byFile = {};
  for (const entry of entries) {
    const file = entry.metadata?.sourceFile || null;
    if (file) {
...
function init() {
  ...
  if (nodeCount > MAX_GRAPH_NODES) {
    process.stderr.write(
      `[INTELLIGENCE] WARN: Graph has ${nodeCount} nodes (>${MAX_GRAPH_NODES}), skipping PageRank\n`,
    );
...
function consolidate() {
  ...
  for (const entry of store) {
    nodes[entry.id] = {
      id: entry.id,
      category: entry.namespace || entry.type || "default",
      confidence: graph?.nodes?.[entry.id]
        ? graph.nodes[entry.id].confidence
        : entry.metadata?.confidence || 0.5,
      accessCount: graph?.nodes?.[entry.id]
  ...
  if (nodeCount > MAX_GRAPH_NODES) {
    process.stderr.write(
      `[INTELLIGENCE] WARN: Graph has ${nodeCount} nodes (>${MAX_GRAPH_NODES}), skipping PageRank in consolidate\n`,
    );
    for (const id of Object.keys(nodes)) pageRanks[id] = 1 / nodeCount;
  } else {
    pageRanks = computePageRank(nodes, edges, 0.85, 30);
...
function stats(outputJson) {
  ...
  const topPatterns = (ranked?.entries || []).slice(0, 10).map((e, i) => ({
    rank: i + 1,
    summary: (e.summary || "").slice(0, 60),
