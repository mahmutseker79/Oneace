...
function getGitInfo() {
  ...
  if (parts.length >= 4) {
    ...
    result.ahead = Number.parseInt(ab[0]) || 0;
    result.behind = Number.parseInt(ab[1]) || 0;
  }
...
function getSwarmStatus() {
  ...
  if (swarmState) {
    ...
    const age = updatedAt ? now - new Date(updatedAt).getTime() : Number.POSITIVE_INFINITY;
  ...
  if (activityData?.swarm) {
    ...
    if (age < staleThresholdMs) {
      return {
        ...
        coordinationActive:
          activityData.swarm.coordination_active || activityData.swarm.active || false,
      };
    }
  }
...
function getSystemMetrics() {
  ...
  if (learningData && learningData.intelligence && learningData.intelligence.score !== undefined) {
    ...
    const fromPatterns =
      learning.patterns > 0 ? Math.min(100, Math.floor(learning.patterns / 20)) : 0;
    const fromVectors =
      agentdb.vectorCount > 0 ? Math.min(100, Math.floor(agentdb.vectorCount / 20)) : 0;
      ...
      if (fs.existsSync(adrPath)) {
        const files = fs
          ...
          .filter(
            (f) =>
              f.endsWith(".md") &&
              (f.startsWith("ADR-") || f.startsWith("adr-") || /^\d{4}-/.test(f)),
          );
...
function getAgentDBStats() {
  ...
  try {
    ...
    if (ranked?.entries && ranked.entries.length > vectorCount) vectorCount = ranked.entries.length;
  } catch {
    /* ignore */
  }
...
function getTestStats() {
  ...
  const testDirNames = ["tests", "test", "__tests__", "src", "v3"];
...
function getIntegrationStatus() {
  ...
  const hasDatabase = [".swarm/memory.db", ".claude-flow/memory.db", "data/memory.db"].some((p) =>
...
function getSessionStats() {
  const sessionPaths = [".claude-flow/session.json", ".claude/session.json"];
...
function generateStatusline() {
  ...
  if (ctxInfo && ctxInfo.usedPct > 0) {
    const ctxColor =
      ctxInfo.usedPct >= 90 ? c.brightRed : ctxInfo.usedPct >= 70 ? c.brightYellow : c.brightGreen;
  ...
  if (agentdb.hasHnsw && agentdb.vectorCount > 0) {
    const speedup =
      agentdb.vectorCount > 10000 ? "12500x" : agentdb.vectorCount > 1000 ? "150x" : "10x";
  ...
  lines.push(
    `${c.brightCyan}\uD83C\uDFD7\uFE0F  DDD Domains${c.reset}    ${progressBar(progress.domainsCompleted, progress.totalDomains)}  ${domainsColor}${progress.domainsCompleted}${c.reset}/${c.brightWhite}${progress.totalDomains}${c.reset}    ${perfIndicator}`,
  );

  // Line 2: Swarm + Hooks + CVE + Memory + Intelligence
  const swarmInd = swarm.coordinationActive
  ...
  const secIcon =
    security.status === "CLEAN"
      ? "\uD83D\uDFE2"
      : security.status === "IN_PROGRESS" || security.status === "STALE"
        ? "\uD83D\uDFE1"
  ...
  const adrColor =
    adrs.count > 0 ? (adrs.implemented === adrs.count ? c.brightGreen : c.yellow) : c.dim;
  const adrDisplay =
    adrs.compliance > 0
  ...
  lines.push(
    `${c.brightPurple}\uD83D\uDD27 Architecture${c.reset}    ${c.cyan}ADRs${c.reset} ${adrDisplay}  ${c.dim}\u2502${c.reset}  ${c.cyan}DDD${c.reset} ${dddColor}\u25CF${String(progress.dddProgress).padStart(3)}%${c.reset}  ${c.dim}\u2502${c.reset}  ${c.cyan}Security${c.reset} ${secColor}\u25CF${security.status}${c.reset}`,
  );

  // Line 4: AgentDB, Tests, Integration
  const hnswInd = agentdb.hasHnsw ? `${c.brightGreen}\u26A1${c.reset}` : "";
  const sizeDisp =
    agentdb.dbSizeKB >= 1024
  ...
  if (integration.mcpServers.total > 0) {
    const mcpCol =
      integration.mcpServers.enabled === integration.mcpServers.total
        ? c.brightGreen
        : integration.mcpServers.enabled > 0
          ? c.brightYellow
          : c.red;
  ...
  if (integration.hasDatabase)
    integStr += `${(integStr ? "  " : "") + c.brightGreen}\u25C6${c.reset}DB`;
  if (integration.hasApi)
    integStr += `${(integStr ? "  " : "") + c.brightGreen}\u25C6${c.reset}API`;
  if (!integStr) integStr = `${c.dim}\u25CF none${c.reset}`;

  lines.push(
    `${c.brightCyan}\uD83D\uDCCA AgentDB${c.reset}    ${c.cyan}Vectors${c.reset} ${vectorColor}\u25CF${agentdb.vectorCount}${hnswInd}${c.reset}  ${c.dim}\u2502${c.reset}  ${c.cyan}Size${c.reset} ${c.brightWhite}${sizeDisp}${c.reset}  ${c.dim}\u2502${c.reset}  ${c.cyan}Tests${c.reset} ${testColor}\u25CF${tests.testFiles}${c.reset} ${c.dim}(~${tests.testCases} cases)${c.reset}  ${c.dim}\u2502${c.reset}  ${integStr}`,
  );
