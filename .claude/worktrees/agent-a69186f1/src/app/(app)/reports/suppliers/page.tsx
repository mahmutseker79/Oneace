...
export default async function SupplierPerformancePage() {
  ...
  const suppliersWithActivity = rows.filter((r) => r.totalPos > 0 || r.receivedValue > 0).length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold">{t.reports.supplierPerformance.heading}</h1>
            <p className="text-muted-foreground">{t.reports.supplierPerformance.subtitle}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button asChild variant="outline">
            <Link href="/reports/suppliers/export">
      ...
      {!hasAnyData ? (
        ...
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">
                  {formatCurrency(totalReceivedValue, { currency: region.currency })}
                </CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>{t.reports.supplierPerformance.kpiTotalPosLabel}</CardDescription>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>{t.reports.supplierPerformance.kpiSuppliersLabel}</CardDescription>
                <CardTitle className="text-2xl">{formatNumber(rows.length)}</CardTitle>
              </CardHeader>
              <CardContent className="text-muted-foreground text-xs">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t.reports.supplierPerformance.detailSubtitle}</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t.reports.supplierPerformance.colSupplier}</TableHead>
                ...
                <TableBody>
                  {rows.map((r) => {
                    const onTimeRate =
                      r.onTimeEligible === 0 ? null : (r.onTimeCount / r.onTimeEligible) * 100;
